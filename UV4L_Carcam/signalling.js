...
function onDataChannel(event) {
    ...
    event.channel.onopen = function () {
        console.log("Channel is really open")
        queryBatteryState();
        queryNightVisionState();
    };
    ...
    event.channel.onmessage = function (event) {
        ...
        if(event.data == "NVSTATE ON") {
            handleNightVisionStateUpdate(event.data)
        }
        if(event.data == "NVSTATE OFF") {
            handleNightVisionStateUpdate(event.data)
        }

        if(event.data.substring(0,8) == "BATSTATE") {
            handleBatteryStateUpdate(parseFloat(event.data.substring(9)))
        }

        //document.getElementById('datareceived').value = event.data;
	    //console.log("^ The above message is for later when we add things like battery indicators >:)");
    };
